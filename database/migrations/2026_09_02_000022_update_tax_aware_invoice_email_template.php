...
<table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-collapse:collapse;border:1px solid #e5e7eb;">
    ...
    @endforeach
    <tr><td style="padding:13px 16px;border-top:1px solid #e5e7eb;color:#6b7280;font-size:13px;">Subtotal</td><td align="right" style="padding:13px 16px;border-top:1px solid #e5e7eb;color:#111827;font-size:13px;font-weight:700;">{{ $formattedSubtotal }}</td></tr>
    <tr><td style="padding:13px 16px;border-top:1px solid #e5e7eb;color:#6b7280;font-size:13px;">{{ $taxName }} {{ $taxPercentage }}%</td><td align="right" style="padding:13px 16px;border-top:1px solid #e5e7eb;color:#111827;font-size:13px;font-weight:700;">{{ $formattedTaxAmount }}</td></tr>
...
{{ $taxName }} {{ $taxPercentage }}%: {{ $formattedTaxAmount }}
...
TEXT,
                'available_variables' => json_encode([
                    ...
                    'taxCountry' => 'Tax country copied to invoice',
                    'taxName' => 'Tax name copied to invoice',
                    'taxPercentage' => 'Tax percentage copied to invoice',
                    'formattedSubtotal' => 'Subtotal with currency label',
                    'formattedTaxAmount' => 'Tax amount with currency label',
                    'formattedAmount' => 'Final total with currency label',
